...
def make_partition(num_bins, lower_inner, upper_inner):
    ...
    if num_bins == 2:
        return np.array([[lower, 0.5*(lower_inner + upper_inner)], [0.5*(lower_inner + upper_inner), upper]])
    ...
    bins_total = np.vstack((np.array([lower, lower_inner]), bins, np.array([upper_inner, upper])))
...
def ppe_simulation(model,
    ...
    if alpha is None:
        param_names = lambd_names + ["alpha"] ## We optimize alpha also
        param_bounds = param_bounds + [[0.001, 70.]]
        
    param_types = ["range"]*len(param_names)
    param_expected_vals = [None]*len(param_names) ## we only focus on the dirichlet log likelihood
